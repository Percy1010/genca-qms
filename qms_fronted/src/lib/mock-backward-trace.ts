...
export function isReverseLeaf(node: ReverseNode): boolean {
  return node.upstream.length === 0
}

/* ==================== 追溯报告数据 ==================== */

export interface ReportChainLevel {
  title: string
  category: Category
  nodes: {
    code: string
...
export interface ReportProductionRecord {
  orderNo: string
  orderType: string
  goodsCode: string
  goodsName: string
...
export interface TraceReportData {
  ...
  warehouse: string
  chainLevels: ReportChainLevel[]
  productionOrders: ReportProductionRecord[]
  inspections: ReportInspection[]
  suppliers: ReportSupplier[]
}

const CAT_ORDER: Category[] = ["成品裸支", "半成品", "原料"]

/** 从起点批次向上收集全链路节点（BFS，去重） */
function collectChain(code: string, batchNo: string): ReverseNode[] {
  const start = queryReverseTrace(code, batchNo)
  if (!start) return []
  const seen = new Set<string>()
  const result: ReverseNode[] = []
  const queue: ReverseNode[] = [start]
  while (queue.length) {
    const n = queue.shift()!
    const key = `${n.sku.code}|${n.inventory.batchNo}`
  ...
  result.sort(
    (a, b) =>
      CAT_ORDER.indexOf(a.sku.category) - CAT_ORDER.indexOf(b.sku.category)
  )
  return result
}

/** 根据成品批次生成完整追溯报告 */
export function buildTraceReport(code: string, batchNo: string): TraceReportData | null {
  const start = queryReverseTrace(code, batchNo)
  if (!start) return null
  const chain = collectChain(code, batchNo)
  ...
  const reportNo =
    reportNoMap[`${code.trim().toUpperCase()}|${batchNo.trim().toUpperCase()}`] ??
    "TR-2026-00015"

  const chainLevels: ReportChainLevel[] = []
  for (const cat of CAT_ORDER) {
    const group = chain.filter((n) => n.sku.category === cat)
    ...
    chainLevels.push({
      ...
      nodes: group.map((n) => ({
        code: n.sku.code,
        name: n.sku.name,
        batchNo: n.inventory.batchNo,
        qty: `${n.inventory.currentQty.toLocaleString()} ${n.inventory.unit}`,
        status: n.inventory.status,
      })),
    })
  }

  const productionOrders: ReportProductionRecord[] = []
  const inspections: ReportInspection[] = []
  for (const n of chain) {
    for (const r of n.stockInRecords) {
      if (r.inspectionNo) {
        inspections.push({
          stage: n.sku.category,
          recordNo: r.inspectionNo,
          result: r.inspectionResult ?? "合格",
          inspector: "质量检验",
          date: r.date,
        })
      }
      productionOrders.push({
        orderNo: r.sourceOrder,
        orderType: r.businessType,
        goodsCode: n.sku.code,
        goodsName: n.sku.name,
        batchNo: n.inventory.batchNo,
        qty: `${r.qty.toLocaleString()} ${r.unit}`,
        date: r.date,
        provider: r.provider,
      })
    }
  ...
  const suppliers: ReportSupplier[] = chain
    .filter((n) => n.sku.category === "原料")
    .map((n) => ({
      material: n.sku.name,
      batch: n.inventory.batchNo,
      supplier: n.sku.defaultProvider,
      cert: "COA 已归档",
    }))
  ...
  return {
    ...
    batchNo: start.inventory.batchNo,
    batchQty: `${start.inventory.currentQty.toLocaleString()} ${start.inventory.unit}`,
    productionDate: start.inventory.productionDate,
    warehouse: start.inventory.warehouse,
    chainLevels,
    productionOrders,
    inspections,
    suppliers,
